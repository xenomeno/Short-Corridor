...
local function GetFuncsMinMaxSizes(funcs, descr)
  ...
  local any_pt = funcs[next(funcs)][1]
...
function DrawGraphs(bmp, funcs_data, descr)
  ...
  for k = 1, div_x do
    ...
    local text = int_x and string.format(axis_x_format, k * size_x // div_x + center_x) or string.format(axis_x_format, k * size_x / div_x + center_x)
